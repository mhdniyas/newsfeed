...
class TrendSyncStateService
{
    public function startCycle(string $source, int $totalCountries, int $totalTopics, int $cycleLimit, int $perCountryLimit): void
    {
        Setting::set('trend_sync_status', 'running');
        Setting::set('trend_sync_requested_at', Setting::get('trend_sync_requested_at') ?: now()->toIso8601String());
        Setting::set('trend_sync_started_at', now()->toIso8601String());
        Setting::set('trend_sync_finished_at', null);
        Setting::set('trend_sync_last_output', null);
        Setting::set('trend_sync_finalize_dispatched', null);
        Setting::set('trend_sync_log', json_encode([
            ...
            [
                'time' => now()->toIso8601String(),
                'level' => 'info',
                'message' => "Trend cycle started. Refreshing keywords and fetching up to {$perCountryLimit} articles for each of {$totalCountries} countries.",
            ],
        ]));
        Setting::set('trend_sync_meta', json_encode([
            'progress' => 4,
            'stage' => 'Refreshing trend keywords',
            'processed_sections' => 0,
            'total_sections' => $totalCountries,
            'processed_topics' => 0,
            'total_topics' => $totalTopics,
            'failed_sections' => 0,
            'article_limit' => $cycleLimit,
            'cycle_limit' => $cycleLimit,
            'section_limit' => $perCountryLimit,
    ...
    {
        return json_decode(Setting::get('trend_sync_meta', '{}') ?: '{}', true) ?: [];
    }

    public function updateMeta(array $payload): void
    {
        Setting::set('trend_sync_meta', json_encode(array_merge($this->meta(), $payload)));
    }
    ...
    public function appendLog(string $message, string $level = 'info'): void
    {
        ...
        Setting::set('trend_sync_log', json_encode(array_slice($log, -80)));
    }

    public function globalLimitReached(): bool
    {
        $meta = $this->meta();
        $limit = (int) ($meta['cycle_limit'] ?? $meta['article_limit'] ?? 90);
    ...
    public function beginSection(NewsSection $section, int $sectionTopics): void
    {
        ...
        $progress = min(92, 10 + (int) floor(($processedSections / $totalSections) * 78));
        ...
        $this->updateMeta([
            ...
            'total_items' => null,
            'current_article' => null,
            'section_limit' => $section->card_limit ?: 10,
            'section_topics' => $sectionTopics,
        ]);

        $this->appendLog("Country {$section->name} started.", 'info');
    }
    ...
    public function updateTopicProgress(NewsSection $section, string $topicName, int $processedTopics, int $totalTopics, int $currentItem = 0, int $totalItems = 0, ?string $articleTitle = null): void
    {
        ...
        $this->updateMeta([
            'progress' => min(95, 12 + (int) floor(($sectionFraction + $topicFraction) * 80)),
            'stage' => "Processing {$section->name}",
            'current_section' => $section->name,
    ...
    public function accumulateStats(array $delta): void
    {
        ...
        $meta['stats'] = $stats;
        Setting::set('trend_sync_meta', json_encode($meta));
    }
    ...
    public function completeSection(NewsSection $section, array $summary): bool
    {
        ...
        $meta['progress'] = min(97, 16 + (int) floor(($processedSections / $totalSections) * 80));
        ...
        Setting::set('trend_sync_meta', json_encode($meta));
        $this->appendLog("Country {$section->name} completed: {$summary['new_articles']} new, {$summary['skipped_duplicates']} duplicates, {$summary['images_recovered']} recovered images.", 'success');
    ...
    public function failSection(NewsSection $section, string $message): bool
    {
        ...
        $meta['total_items'] = null;
        $meta['current_article'] = null;
        $meta['progress'] = min(97, (int) ($meta['progress'] ?? 0) + 5);

        Setting::set('trend_sync_meta', json_encode($meta));
        $this->appendLog("Country {$section->name} failed: {$message}", 'error');
    ...
    public function finalizeCycle(): string
    {
        ...
        $summary = "Trend cycle completed. Saved "
            . ((int) ($stats['new_articles'] ?? 0)) . '/' . ((int) ($meta['cycle_limit'] ?? 90))
            . " new articles across " . ((int) ($meta['processed_sections'] ?? 0)) . '/' . ((int) ($meta['total_sections'] ?? 0))
            . " countries, skipped " . ((int) ($stats['skipped_duplicates'] ?? 0))
            . " duplicates, recovered " . ((int) ($stats['images_recovered'] ?? 0)) . ' images.';
        ...
        Setting::set('trend_sync_status', $status);
        Setting::set('trend_sync_finished_at', now()->toIso8601String());
        Setting::set('trend_sync_last_output', $summary);
        Setting::set('trend_sync_process_id', null);
        Setting::set('trend_sync_total_runs', (string) (((int) Setting::get('trend_sync_total_runs', '0')) + 1));
        Setting::set('trend_sync_last_success_at', now()->toIso8601String());
