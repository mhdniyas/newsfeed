...
@section('meta_description', 'Official Kerala lottery result page with all prize numbers — 1st through 8th prize, consolation prizes, and PDF download.')

@section('styles')
<style>
/* ── Lottery Show Page Styles ── */
.lottery-hero        { background: linear-gradient(135deg, #0f172a 0%, #1e3a5f 60%, #0e7490 100%); }
.prize-card          { border-radius: 1.25rem; border: 1px solid rgba(255,255,255,.08); background: rgba(255,255,255,.04); backdrop-filter: blur(8px); transition: transform .2s, box-shadow .2s; }
.prize-card:hover    { transform: translateY(-2px); box-shadow: 0 8px 32px rgba(0,0,0,.25); }
...
.prize-cons          { border-radius: 1rem; border: 1px solid rgba(99,102,241,.25); background: rgba(99,102,241,.07); }
.ticket-badge        { display: inline-flex; align-items: center; border-radius: 9999px; border: 1px solid rgba(255,255,255,.2); background: rgba(255,255,255,.1); padding: .2rem .65rem; font-size: .75rem; font-weight: 700; letter-spacing: .04em; font-family: monospace; }
...
.status-failed       { border: 1px solid #fca5a5; background: #fff1f2; color: #b91c1c; }
...
<div class="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8 py-6 space-y-5">
        ...
        <div class="px-5 sm:px-8 pt-7 pb-6">
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <h1 class="mt-2 text-2xl sm:text-3xl font-extrabold text-white leading-tight truncate">
    ...
    @elseif($result->hasParsedPrizes() || !empty($result->consolation_prizes) || !empty($result->other_prizes))
            ...
            <div class="section-card">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-black text-slate-900">Full Ticket Numbers</p>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2">
                    @foreach($result->consolation_prizes as $ticket)
                        <span class="inline-flex items-center rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-800 font-mono tracking-wider">
                            {{ $ticket }}
                        </span>
                    @endforeach
                </div>
        ...
        {{-- ── Other Prizes (4th–8th) ── --}}
        @if(!empty($result->other_prizes))
            @foreach($result->other_prizes as $prize)
                <div class="section-card">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
                        <div>
                            <p class="text-[10px] font-bold uppercase tracking-[.2em] text-slate-400">Ending Numbers</p>
